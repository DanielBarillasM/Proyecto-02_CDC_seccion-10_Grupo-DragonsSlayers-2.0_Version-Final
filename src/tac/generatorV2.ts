...
class TacGenerator {
  ...
  private literal(ctx: LiteralExpressionContext): TacOperand {
    ...
    if (ctx.StringLiteral()) {
      ...
      try { value = JSON.parse(ctx.text) as string; } catch { /* conserva el texto sin comillas */ }
...
export function generateTacV2(program: ProgramContext, semantic: SemanticAnalysisResult): TacGenerationResult {
